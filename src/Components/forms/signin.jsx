...
const signin = () => {
    ...
    const handleLogin = async () => {
        setLoading(true)
        try {
            const response = await axios.post(`${API_BASE_URL}/api/v1/auth/login`, null, {
    ...
    return (
        <div className='body' style={{ display: "flex", justifyContent: "flex-end", alignItems: "center" }}>
                ...
                <div>
                    ...
                    <form onSubmit={handleSubmit} style={{ padding: "5px", display: "flex", alignItems: "center", marginTop: "20px" }}>
                        <div className='input-sign'>
                            <InputF error={emailError} value={email} onChange={(e) => setEmail(e.target.value)} type="email" label="Enter Email" placeHolder="johndeo@gmail.com" />
                        </div>
                        <div className='input-sign'>
                            <InputF error={passwordError} value={password} onChange={(e) => setPassword(e.target.value)} type="password" label="Enter Password" placeHolder="***********" />
            ...
            {popup && (
            <Popup subject='Login Successful' buttonText='Proceed to Cardboard' linkTo={linkToLogin}/>
          )}
        </div>
    )
}
